...
exports.compositeProductOnBackground = compositeProductOnBackground;
 ...
 */
async function compositeProductOnBackground(productBuffer, backgroundBuffer) {
    console.log('Compositing cleaned product cutout onto generated background...');
    try {
        const startTime = Date.now();
        const background = await jimp_1.Jimp.read(backgroundBuffer);
        const product = await jimp_1.Jimp.read(productBuffer);
        ...
        const shadow = createContactShadow(Math.max(20, Math.round(newWidth * 0.72)), Math.max(8, Math.round(newHeight * 0.055)));
        ...
        console.log(`Product placed naturally: cutout=${prodWidth}x${prodHeight}, resized=${newWidth}x${newHeight}, x=${x}, y=${y}, surfaceY=${surfaceY}`);
...
function cleanSmallAlphaComponents(image) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (visited[start] || alphaAt(start) < ALPHA_THRESHOLD)
                continue;
            const component = [];
            const queue = [start];
            visited[start] = 1;
            for (let cursor = 0; cursor < queue.length; cursor++) {
                ...
                for (const [neighborX, neighborY] of neighbors) {
                    ...
                    if (visited[neighbor] || alphaAt(neighbor) < ALPHA_THRESHOLD)
                        continue;
                    visited[neighbor] = 1;
                    queue.push(neighbor);
                }
...
function createContactShadow(width, height) {
    const shadow = new jimp_1.Jimp({ width, height, color: 0x00000000 });
    const centerX = (width - 1) / 2;
    const centerY = (height - 1) / 2;
    shadow.scan(0, 0, width, height, function (x, y, index) {
...
function tuneChannel(value, gain) {
